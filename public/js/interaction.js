...
var text_all = getAll(".text-wrapper");
...
dots.forEach(function(dot, index, array){
	...
	dot.addEventListener("click", function(){
		...
		if(!this.classList.contains("active")) {
            ...
            animateText(getIndex(activeDot, thisArray), getIndex(this, thisArray), dynamicAnimProps.direction, dynamicAnimProps.translateVal);
		}
...
function animateText(oldIndex, newIndex, direction, translateVal){

    var tl = new TimelineMax({
        onComplete: function(){
            // remove transform property
            text_all[oldIndex].style.transform = "";
            text_all[newIndex].style.transform = "";
        }
    });
    ...
    // animate the active image
        ...
        // set the clicked image translateX position
        .set(text_all[newIndex], {
            [dynamicAnimProps.imageDirection]: translateVal,
        }, .15)

        // animate the cliced image
        .to(text_all[newIndex], .3, {
            [dynamicAnimProps.imageDirection]: 0,
            opacity: 1,
        })
